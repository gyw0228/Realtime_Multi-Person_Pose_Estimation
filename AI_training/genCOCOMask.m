...
for mode = 0:1 %mode 1 for validation
    ...
    for i = 1:L
        ...
        try
            ...
            [h,w,~] = size(imread(['dataset/COCO/', img_paths])); %read origin image
            mask_all = false(h,w);
            mask_miss = false(h,w); %h * w zero
            flag = 0;
            for p = 1:length(coco_kpt(i).annorect)
                ...
                [X,Y] = meshgrid( 1:w, 1:h ); %output w*h
                mask = inpolygon( X, Y, seg(1:2:end), seg(2:2:end)); % get segmentation mask which in is 1
            ...
            if flag == 1
                mask_miss = not(or(mask_miss,mask_crowd)); %has no 1 to be 1
                mask_all = or(mask_all, mask_crowd);
            else
                mask_miss = not(mask_miss);
            end
            
            coco_kpt(i).mask_all = mask_all; %get segmentation area
            coco_kpt(i).mask_miss = mask_miss; %get not segmentation area
