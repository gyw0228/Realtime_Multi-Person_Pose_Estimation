...
caffe_path = '/home/haoquan/HardDisk/yihongwei/caffe_train/'
import sys, os
...
def setLayers_twoBranches(data_source, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=False, batchnorm=0, lr_mult_distro=[1,1,1]):
    ...
    # produce data definition for deploy net
        ...
        n.tops[label_name[2]], n.tops[label_name[3]], n.tops[label_name[4]], n.tops[label_name[5]] = L.Slice(n.label, slice_param=dict(axis=1, slice_point=[26, num_parts+1, num_parts+27]), ntop=4)
    ...
    for l in range(0, len(layername)):
        ...
        if layername[l] == 'C':
        ...
        elif layername[l] == 'L2': # for layer L2
            # Loss: n.loss layer is only in training and testing nets, but not in deploy net.
            ...
            for level in range(0,2):
                if deploy == False and "lmdb" not in data_source:
                    ...
                    n.tops['accuracy_stage%d_L%d' % (stage, level+1)] = L.Accuracy(n.tops[name], n.tops[label_name[level]], include=dict(phase=caffe.TEST))
...
--weights=%s/models/vgg/VGG_ILSVRC_19_layers.caffemodel \
2>&1 | tee ./output.txt' % (caffe_path,  caffe_path))

if __name__ == "__main__":

    parser = argparse.ArgumentParser()
    parser.add_argument('--exp', type=int, default=1, help='exp number[1-4]')
    parser.add_argument('--inputsize', type=str, default='304,304', help='exp number[1-4]') #default for size
    ...
    if(exp == 1):
#directory for 
        directory = '/mnt/sda1/yihongwei/dataset/AIChallenger/ai_challenger_exp_caffe/pose40_val/val/'
#serverFolder for store all prototxt
        serverFolder = '/mnt/sda1/yihongwei/dataset/AIChallenger/ai_challenger_kpt_model/pose40_val/val'
#base_folder for store model        
	base_folder = directory+'model'
## dataFolder and source for input lmdb
        dataFolder = '/mnt/sda1/yihongwei/dataset/AIChallenger/ai_challenger_keypoint_train_20170902/AIChallenger_lmdb_20170902' #val_lmdb
        val_dataFolder = '/mnt/sda1/yihongwei/dataset/AIChallenger//ai_challenger_keypoint_validation_20170911/AIChallenger_val_lmdb_20170911' #val_lmdb
     
        source = '/mnt/sda1/yihongwei/dataset/AIChallenger/ai_challenger_keypoint_train_20170902/AIChallenger_lmdb_20170902'
        val_source = '/mnt/sda1/yihongwei/dataset/AIChallenger//ai_challenger_keypoint_validation_20170911/AIChallenger_val_lmdb_20170911' #val_lmdb
        ...
        transform_param = dict(stride=8, crop_size_x=368, crop_size_y=368,
                                 ...
                                 visualize=False, np_in_lmdb=14, num_parts=np)
        nCP = 3
