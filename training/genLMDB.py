...
def writeLMDB(datasets, lmdb_path, validation):
	...
	for count in range(numSample):
		...
		if (data[idx]['isValidation'] != 0 and validation == 1): # validation used for test not int training
		...
		# total 7+4*nop lines
		if "COCO" in data[idx]['dataset']: # 3-dimension concatenate
		...
		img4ch = np.transpose(img4ch, (2, 0, 1)) #transpose to mask_miss, img, meta_data
		print img4ch.shape
